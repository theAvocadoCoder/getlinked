...
function Navbar() {
  ...
  return (
    <nav className="flex items-center w-full justify-between px-12 lg:px-16 xl:px-32 pt-[1.875rem] pb-[1.44rem] lg:pt-[4.1875rem]">
      ...
      <button type="button" className={`w-6 h-6 bg-[url('assets/burger.svg')] bg-contain bg-no-repeat flex justify-center items-center text-center p-0 lg:hidden`} aria-label="Open Menu" onClick={toggleSideBar} />
      ...
      <div className={`fixed flex flex-col justify-center gap-10 w-screen h-screen px-16 md:px-32 md:text-[1.5rem] top-0 left-0 font-medium lg:text-[1.125rem] 2xl:text-[1.75rem] lg:w-[min(68%,40rem)] xl:w-[min(65%,58rem)] lg:h-fit lg:flex-row lg:gap-0 lg:justify-end lg:static lg:p-0 ${isSidebarOpen ? "flex bg-indigo-gl lg:bg-transparent" : "hidden lg:flex"}`}>
        <button type="button" className={`w-6 h-6 bg-[url('assets/exit.svg')] bg-contain bg-no-repeat flex justify-center items-center text-center p-0 lg:hidden absolute top-16 right-16`} aria-label="Close Menu" onClick={toggleSideBar} />
